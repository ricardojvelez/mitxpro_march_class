  ...
  return (
    <div className="app">
      ...
      <div className="stage">
        ...
        {bracket.quarterFinals.map((team) => (
          <button
            key={team.id}
            onClick={() => moveTeamUp(team, "quarterFinals")}
          >
            {team.name}
          </button>
        ))}
      ...
      <div className="stage">
        <h2>Semi Finals</h2>
        {bracket.semiFinals.map((team) => (
          <button key={team.id} onClick={() => moveTeamUp(team, "semiFinals")}>
            {team.name}
          </button>
        ))}
      </div>
      <div className="stage">
        <h2>Final</h2>
        {bracket.final.map((team) => (
          <button key={team.id} onClick={() => moveTeamUp(team, "final")}>
            {team.name}
          </button>
        ))}
      </div>
